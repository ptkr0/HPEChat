		...
		public async Task<ServerMessageDto> Handle(SendServerMessageCommand request, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				if (request.Attachment != null)
				{
					...
					uploadedFilePath = await _fileService.UploadFile(request.Attachment);
					...
					if (attachmentType == AttachmentType.Image)
					{
						...
						uploadedPreviewPath = await _fileService.GenerateAndUploadPreviewImage(request.Attachment);
				...
				await _serverHub
					...
					.MessageAdded(channel.ServerId, messageDto);
