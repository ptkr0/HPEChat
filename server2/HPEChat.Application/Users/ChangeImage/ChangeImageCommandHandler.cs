...
namespace HPEChat.Application.Users.ChangeAvatar
{
	internal class ChangeImageCommandHandler : IRequestHandler<ChangeImageCommand, UserInfoDto>
	{
		...
		public async Task<UserInfoDto> Handle(ChangeImageCommand request, CancellationToken cancellationToken)
		{
			...
			var updatedUserDto = request.Image == null
				...
				: await HandleImageUpdateAsync(user, request.Image);

			await _userHub
					.Clients
					.All
					.AvatarChanged(updatedUserDto);
		...
		private async Task<UserInfoDto> HandleImageUpdateAsync(User user, IFormFile newAvatar)
		{
			// 1. upload new image
			var newImagePath = await _fileService.UploadAvatar(newAvatar, user.Id);
